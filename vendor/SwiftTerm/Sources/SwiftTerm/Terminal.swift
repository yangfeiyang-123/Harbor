...
//                break
    ...
    /// Rendered text with soft-wrapped rows joined, suitable for reconnect snapshots.
    /// Escape sequences and non-text terminal state are deliberately excluded.
    public func getRecoveryText(kind: BufferKind = .active) -> String {
        let b = bufferFromKind(kind: kind)
        var result = ""
        for row in 0..<b.lines.count {
            let line = b.lines[row]
            if row > 0 && !line.isWrapped { result += "\n" }
            result += line.translateToString(trimRight: true, skipNullCellsFollowingWide: true, characterProvider: { self.getCharacter(for: $0) }).replacingOccurrences(of: "\u{0}", with: " ")
        }
        while result.last == "\n" { result.removeLast() }
        return result
    }
