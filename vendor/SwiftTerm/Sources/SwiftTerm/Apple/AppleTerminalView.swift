...
extension TerminalView {
    ...
    func resetFont()
    {
        ...
        if frame.width >= cellDimension.width * 2 && frame.height >= cellDimension.height * 2 {
    ...
    /// Repaint the current buffer without changing fonts, terminal modes,
    /// selection, scrollback or the remote process's window size.
    ...
    func processSizeChange (newSize: CGSize) -> Bool {
        if newSize.width < cellDimension.width * 2 || newSize.height < cellDimension.height * 2 {
            return false
        }
    ...
    {
        search.invalidate()
        // On macOS, incoming output must not cancel a mouse selection before
        // the user can copy it. The terminal buffer tracks affected ranges.
        #if !os(macOS)
